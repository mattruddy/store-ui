...
export const getPWAs = async (page: number, category?: string) => {
  try {
    const response = await Axios.request({
      url: category
        ? `${vars().env.API_URL}/public/pwas/${page}/${category}`
        : `${vars().env.API_URL}/public/pwas/${page}`,
      method: "GET",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
...
export const postRating = async (
  ...
  try {
    const response = await Axios.request({
      url: `${vars().env.API_URL}/${
        isAuth ? "secure" : "public"
      }/pwa/rating/${appId}`,
      method: "POST",
      headers: header,
      data: body,
...
export const postAddScreenshots = async (
  screenshots: File[],
  appId: number
) => {
  const token = await Storage.get({ key: TOKEN })
